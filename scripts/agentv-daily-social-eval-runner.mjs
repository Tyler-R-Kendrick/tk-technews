...
const canonicalEvalPath = path.join(root, 'evals', 'daily-rich-social', 'EVAL.yaml');
const targetsPath = path.join(root, '.agentv', 'targets.yaml');
const generatedEvalPath = path.join(root, 'evals', 'daily-rich-social', 'daily-rich-social.agentv.eval.yaml');
...
if (command === 'validate') {
  ...
  runAgentv([
    ...
    targetsPath,
    '--target',
    'tk-technews-daily-social-fixtures',
    '--threshold',
    '1',
    '--workers',
    '1',
    '--output',
    path.join(root, '.agentv', 'results', 'daily-rich-social')
  ]);
} else {
  throw new Error(`Unknown daily social eval command: ${command}`);
}
...
function validateCanonicalEval(evalFile, evalDir) {
  ...
  for (const test of evalFile.tests) {
    if (!test.id) throw new Error('Every daily social eval test needs an id.');
    const assertions = test.assert ?? test.assertions;
    if (!Array.isArray(assertions) || assertions.length === 0) throw new Error(`Test ${test.id} must use canonical assert entries.`);
...
function validateAssertions(assertions, evalDir, location) {
  for (const assertion of assertions) {
    ...
    if (assertion.type === 'code_judge' || assertion.type === 'code-grader') {
      ...
      if (scriptPath?.endsWith('.mjs') || scriptPath?.endsWith('.js')) assertFileExists(path.join(evalDir, scriptPath), `${location}.${assertion.name}.script`);
    }
...
function runAgentv(args) {
  ...
  const spawnArgs = process.platform === 'win32'
    ? ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', ['&', psQuote(path.join(root, 'node_modules', '.bin', 'agentv.cmd')), ...args.map(psQuote)].join(' ')]
    : ['agentv', ...args];
  const result = spawnSync(executable, spawnArgs, { cwd: root, stdio: 'inherit', shell: false });
